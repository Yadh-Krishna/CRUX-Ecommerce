...
const productDetails = async (req, res, next) => {
  try {
    ...
    if (
      (product.category && product.category.isBlocked) ||
...
const loadHome = async (req, res, next) => {
  try {
    const user = req.user;
    const validCategories = await Category.find({ isDeleted: false }).select(
      "_id"
    );
    const validBrands = await Brand.find({ isDeleted: false }).select(
      "_id image name"
    );
...
const productList = async (req, res, next) => {
  try {
    ...
    if (brand) {
      ...
      query.brands = {
        $in: selectedBrands.filter((b) => validBrandIds.includes(b)),
      };
    }
    ...
    if (category) {
      const selectedCategories = Array.isArray(category)
        ...
        : [category];
      query.category = {
        $in: selectedCategories.filter((c) => validCategoryIds.includes(c)),
      };
    }
