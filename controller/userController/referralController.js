...
const loadReferrals = async (req, res) => {
  try {
    ...
    let referral = await Referral.findOne({ userId: user }).populate(
      "referredUsers"
    );
    if (!referral) {
    ...
    res
      ...
      .render("referrals", { referral, user: loggedInUser });
  } catch (err) {
  ...
  try {
    ...
    if (users) {
      return res
        ...
        .json({ success: false, message: "Email already available" });
    }
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      return res
        .status(statusCodes.SUCCESS)
        .json({ success: true, message: "Invitation mail sent successfully" });
    });
  } catch (err) {}
};
