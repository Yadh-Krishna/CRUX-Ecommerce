...
const loadProfile = async (req, res) => {
  try {
    ...
    res
      ...
      .json(errorMessages.SERVER.SERVER_ERROR);
  }
};
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (user.email === email) {
      return res
        .status(400)
        .json({ success: false, message: "No change in email" });
    }
    if (!user.isActive) {
      return res
        .status(403)
        .json({ success: false, message: errorMessages.USER.BLOCKED });
    }
    const users = await User.findOne({ email });
    if (users) {
      return res
        .status(409)
        .json({ success: false, message: errorMessages.USER.ALREADY_EXISTS });
    }
    ...
    res
      .status(errorMessages.SERVER.SERVER_ERROR)
  ...
  try {
    ...
    if (!user) {
      return res
        ...
        .json({ success: false, message: errorMessages.USER.NOT_FOUND });
    }
...
const defaultAddress = async (req, res) => {
  try {
    ...
    res
      .status(statusCodes.SUCCESS)
      .json({ success: true, message: "Address defaulted successfully" });
  } catch (err) {
    console.log(err);
    res
      .status(statusCodes.SERVER_ERROR)
...
const addWishlist = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(statusCodes.NOT_FOUND)
        .json({ success: false, message: "Unauthorised!!, Try Logging in!!" });
    }
    const user = await User.findById(userId);
    ...
    if (!user) {
      return res
        .status(statusCodes.NOT_FOUND)
        .json({ success: false, message: "User not found, Try Logging in!!" });
    }
    ...
    if (cart) {
      return res
        .status(406)
        .json({ success: false, message: "Product already available in cart" });
    }
