  ...
  try {
    ...
    res.render("sales-report", {
      ...
      paymentMethods: Object.keys(paymentMethods).map((key) => ({
        name: key,
        count: paymentMethods[key],
      })),
      // coupons: Object.keys(coupons).map(key => ({ code: key, totalDiscount: coupons[key] })),
      filterParams: `&period=${period}&dateFrom=${dateFrom || ""}&dateTo=${
        dateTo || ""
      }&search=${search || ""}`,
    });
  } catch (error) {
  ...
  if (query.period && query.period !== "custom") {
    ...
    switch (query.period) {
      ...
      case "weekly":
        ...
        ).setHours(0, 0, 0, 0); // Start of the week
        endDate = new Date().setHours(23, 59, 59, 999);
        break;
      case "monthly":
        startDate = new Date(today.getFullYear(), today.getMonth(), 1).setHours(
          0,
          0,
          0,
          0
...
const pdfExport = async (req, res, next) => {
  try {
    ...
    doc
      .fontSize(22)
      .font("Helvetica-Bold")
      .text("CRUX E-Commerce", { align: "center" });
    doc
      .fontSize(16)
      .font("Helvetica")
    ...
    if (req.query.period)
      filterInfo += `Period: ${req.query.period.toUpperCase()}, `;
    if (req.query.dateFrom && req.query.dateTo)
      filterInfo += `Date Range: ${req.query.dateFrom} - ${req.query.dateTo}, `;
...
const excelExport = async (req, res, next) => {
  try {
    ...
    if (req.query.period)
      filterText += `Period: ${req.query.period.toUpperCase()}, `;
    if (req.query.dateFrom && req.query.dateTo)
      filterText += `Date Range: ${req.query.dateFrom} - ${req.query.dateTo}, `;
