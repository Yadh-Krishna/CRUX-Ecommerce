...
const blockCategory = asyncHandler(async (req, res) => {
  ...
  if (!category) {
    ...
    return res
      .status(404)
      .json({ success: false, message: req.flash("error")[0] });
  }
  ...
  try {
    ...
    if (!category) {
      ...
      for (let product of products) {
        const productDiscount = product.offerApplied
          ? product.prodOffer || 0
          : 0;
        const categoryDiscount = category.offerApplied
          ? category.catOffer || 0
          : 0;
    ...
    return res
      .status(200)
      .json({ success: true, message: "Offer applied successfully!" });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
