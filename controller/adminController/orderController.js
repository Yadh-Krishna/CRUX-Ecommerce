...
const orderList = async (req, res) => {
  try {
    const { search="", status="Status", page = 1, limit = 10 } = req.query;
    ...
    //     { "user.fullName": { $regex: search, $options: "i" } }, // Name search
...
//       // Search only orderId in DB, filter fullName/email in JS later
//       if (search) {
//         query.orderId = { $regex: search, $options: "i" };
//       }
...
//       // Filter user fields after population
//       if (search) {
//         const lowerSearch = search.toLowerCase();
//         orders = orders.filter(order =>
//           order.orderId.toLowerCase().includes(lowerSearch) ||
//           order.user?.fullName?.toLowerCase().includes(lowerSearch) ||
