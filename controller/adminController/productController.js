...
const updateProduct = asyncHandler(async (req, res) => {
  try {
    ...
    const categoryDiscount = product.category.offerApplied
      ? product.category.catOffer
      : 0;
    ...
    if (req.files?.replacedImages) {
      const newImages = req.files.replacedImages.map(
    ...
    if (req.files?.addImages) {
      const additionalImages = req.files.addImages.map(
        (file) => `/uploads/products/${file.filename}`
      );
    ...
    if (updatedImages.length > 5) {
      return res
        .status(400)
        .json({
          errors: { addImage: "You can only upload a maximum of 5 images." },
        });
    }
...
const removeOffer = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({ success: true, message: "Offer is Removed" });
  } catch (err) {
    console.error(err);
    return res
